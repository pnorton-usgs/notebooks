...
# %%
st_date = datetime.datetime(2000,1,1)    # v1.0 used 1990-01-01 to 1999-12-31
en_date = datetime.datetime(2009,12,31)  # v1.1 used 2000-01-01 to 2009-12-31

# %% [markdown]
# ### Read the baseline information

# %%
# Baseline recharge has annual timestep
# All recharge variables are normalized 0.0 to 1.0
base_df = xr.open_dataset(baseline_file)
base_df = base_df.assign_coords(nhru=(base_df.nhm_id))
base_df = base_df[[max_var, min_var, 'nhru']].sel(time=slice(st_date, en_date))
# base_df
...
# %% [markdown]
# #### Compute annual cumulative recharge from daily model output

# %%
# Compute the monthly mean from the simulation daily output
# 1AS - resample to annual total from the start of each year
sim_ann_cum = sim_df.resample(time='1AS').sum()
# sim_ann_cum

# %% [markdown]
# ### Compute the by-HRU counts of simulation annual values that are within baseline bounds

# %%
ss = sim_ann_cum[sim_var].loc[:, hru_order].data
...
bb_min = base_df[min_var].loc[:, hru_order].data
bb_max = base_df[max_var].loc[:, hru_order].data
...
# %% [markdown]
# ### Add the annual sum as percent within bounds

# %%
new_param = 'fit_recharge_ann'
# pdb.parameters.remove(new_param)

pdb.parameters.add(name=new_param, datatype=2, minimum=0.0, maximum=100.0, default=0.0,
                   description='Percent of annual recharge values within baseline bounds')
