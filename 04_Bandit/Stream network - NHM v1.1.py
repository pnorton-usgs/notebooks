...
# %%
# workdir_tb = '/Users/pnorton/Projects/National_Hydrology_Model/Trans-boundary_HRUs/2019-11-25_work'
workdir_tb = '/Users/pnorton/tmp/tmp_paramdb'

tosegment_nhm = read_csv('{}/tosegment_nhm.csv'.format(workdir_tb))
nhm_seg = read_csv('{}/nhm_seg.csv'.format(workdir_tb))

# tb_cutoff_segs = read_csv('{}/seg_id_nat_v1.csv'.format(workdir_tb))

# tosegment_nhm = read_csv('{}/GFv11_tosegment.csv'.format(workdir_tb))
# nhm_seg = read_csv('{}/GFv11_seg.csv'.format(workdir_tb))                       
...
# %%
workdir_nhm = '/Users/pnorton/Projects/National_Hydrology_Model/datasets/bandit/nhmparamdb_tb_work'

# tosegment_nhm = read_csv('{}/tosegment_nhm.csv'.format(workdir_nhm))
# nhm_seg = read_csv('{}/nhm_seg.csv'.format(workdir_nhm))

# nhm_seg_to_idx = 

# %%
# %time pdb = ParamDb(paramdb_dir=workdir_nhm, verbose=True, verify=True)
...
# %% [markdown]
# ### Add TB stream network nodes to existing graph

# %%
# Build the stream network
include_hrus = False

for ii, vv in enumerate(tb_tosegment_nhm):
    if vv == 0:
        dag_streamnet.add_edge(tb_seg_id_nhm[ii], 'Out_{}'.format(tb_seg_id_nhm[ii]))
        num_outlets += 1
    else:
        dag_streamnet.add_node(tb_seg_id_nhm[ii], style='filled', fillcolor='yellow')
        dag_streamnet.add_node(tb_tosegment_nhm[ii], style='filled', fillcolor='yellow')
        dag_streamnet.add_edge(tb_seg_id_nhm[ii], tb_tosegment_nhm[ii])

    if include_hrus:
        # Now add the HRUs
        nr_cnt = 0

        for ii, vv in enumerate(tb_hru_segment_nhm):
            hru_node = 'H_{}'.format(tb_hru_id_nhm[ii])
            dag_streamnet.add_node(hru_node, style='filled', fillcolor='yellow')
...
# for xx in nx.simple_cycles(dag_streamnet):
#     for yy in xx:
#         dag_streamnet_tb.nodes[yy]['style'] = 'filled'
#         dag_streamnet_tb.nodes[yy]['fillcolor'] = 'orange'
#     print(xx)        
print("Number of subset nodes: {}".format(dag_streamnet.number_of_nodes()))
print("Number of subset edges: {}".format(dag_streamnet.number_of_edges()))
print("Number of outlet nodes: {}".format(num_outlets))
...
# Connect the new segments flowing from TB to NHM
# for tb_node, nhm_node in iteritems(tb_out):
#     dag_streamnet.nodes[nhm_node]['fontcolor'] = 'white'
#     dag_streamnet.nodes[nhm_node]['style'] = 'filled'    
...
plot = nx.nx_pydot.to_pydot(dag_ds_subset)
plot.write_pdf('{}/tb_seg_{}.pdf'.format(workdir_tb, dsmost_seg[0]))
...
# %%
# Add the HRUs from the transboundary  HRUs
for xx in tb_hru_segment_nhm:
    hru_segment_nhm_new.append(xx)
    
for xx in tb_hru_id_nhm:
    nhm_id_new.append(xx)
