...
filename = f'{workdir}/gsflow_grid_v2.params'
plot_filename = f'{workdir}/sagehen_gsflow_grid.png'
...
# plot_filename = '{}/sagehen_grid_GW.png'.format(workdir)
...
gw_up_id = pfile.parameters['gw_up_id'].data
gw_down_id = pfile.parameters['gw_down_id'].data
gw_pct_up = pfile.parameters['gw_pct_up'].data
gw_strmseg_down_id = pfile.parameters['gw_strmseg_down_id'].data

# Build the network
dag_uphru = nx.DiGraph()
for ii, vv in enumerate(gw_up_id):
    # ii => cascade number
    # vv => hru id
    dag_uphru.add_node(vv)
    
    if gw_strmseg_down_id[ii] == 0 and gw_pct_up[ii] > 0.0:
        dag_uphru.add_node(gw_down_id[ii])
        dag_uphru.add_edge(vv, gw_down_id[ii])
    else:
        # GWR drains to a stream segment
        strm_node = 'S_{}'.format(gw_strmseg_down_id[ii])
