...
for basinid in basinids:
    ...
    st_model = to_datetime(cfg['start_date_model']['value'])
    ...
    os.chdir('%s/%s' % (workdir, best_modelrunid))
    
    # Update the control file
    #cobj = prms.control('%s/%s/%s' % (workdir, best_modelrunid, cfg['prms_control_file']['value']))
    cobj = prms.control('%s' % cfg['prms_control_file']['value'])
    sv_vals = cobj.get_var('statVar_names')[0]['values']
    
    # Change basin_orad to basin_potsw
    sv_vals[sv_vals.index('basin_orad')] = 'basin_potsw'
    cobj.replace_values('statVar_names', sv_vals)
    cobj.write_control_file('%s/%s/%s' % (workdir, best_modelrunid, cfg['prms_control_file']['value']))
    
    # Re-run PRMS for this modelrunid
    cmd_opts = " -C%s -set param_file %s -set start_time %s -set end_time %s" % (cfg['prms_control_file']['value'], cfg['prms_input_file']['value'], to_prms_datetime(st_model), to_prms_datetime(en))
    subprocess.call(cfg['cmd_prms']['value'] + cmd_opts, shell=True)
